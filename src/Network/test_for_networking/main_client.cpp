...
ssize_t handle_request(char* receive_buffer){
    receive_index++;
    if(receive_index%100000==0){
    // just print 35 bytes of every message.
        cout<<"*echo client* ACK ID is : "<<receive_index<<" . Msg is : "<<string(receive_buffer,receive_buffer+35)<<endl;
    }
...
public:
    test_class( int session_num, int thd_num){
        echo_client = new Connection( handle_request, session_num, thd_num);
    }
